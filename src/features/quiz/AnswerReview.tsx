...
import type { Question, LearningMetadata } from "@/types/question";
import type { Attempt } from "@/types/attempt";
import { ResultBadge } from "@/components/Badges";

export function LearningPanel({ learning }: { learning: LearningMetadata }) {
  ...
  return (
    <div className="card p-3.5" style={{ background: "#1C242B" }}>
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        ...
        <div className="mt-2.5">
          ...
          {rows.map(([label, val]) => (
            <div key={label} className="mb-2">
              ...
              <div className="text-[13px]">{val}</div>
            </div>
          ))}
        </div>
      )}
...
export function AnswerReview({
  ...
  const selectedArr = Array.isArray(attempt.selected_answers)
    ? attempt.selected_answers
    : [attempt.selected_answers];

  const selected = new Set(
    (selectedArr as (string | number | null | undefined)[]).map(String)
  );
  ...
  return (
    <div className="mt-4.5 border-t border-border pt-4">
      <div className="flex items-center gap-2.5 mb-3">
        ...
        <span className="text-xs text-textDim">{attempt.time_taken}s</span>
      </div>

      {opts.length > 0 && (
        <div className="mb-3.5">
          {opts.map((o) => {
            const wasSelected = selected.has(String(o.id));
      ...
      {!opts.length && (
        <div className="text-[13px] mb-3.5">
          Correct answer: {JSON.stringify(attempt.correct_answers)}
        </div>
      )}

      {question.learning && <LearningPanel learning={question.learning} />}
    </div>
  );
}